...
import TD_Setup.TD_SR_Levels as TD
...
class TradingStrategy:
    def __init__(self):
        ...
        self.LOT_SIZE = 0.01
        self.levels = {'SUPPORT': 0, 'RESISTANCE': 0, 'ATR': 0}
        self.LONG = False
        self.SHORT = False
        self.ENTRY = 0
        self.STOP_LOSS = 0
    ...
    def perform_task(self):
        ...
        try:
            ...
            data_df.to_csv('TDSR_output.csv', sep='\t', index=False)
        ...
        if self.LONG:
            self.STOP_LOSS = self.trailing_stops(self.LONG, self.SHORT,candle_high,candle_low, candle_close ,self.ENTRY, self.STOP_LOSS, self.sl_points, self.BREAK_EVEN_POINT)
    ...
    def connect(self):
        while True:
            ...
            if self.prev_minute != now.strftime("%H:%M"):
                ...
                if self.print_flag:
                    logger.info(f'---( TDSR )---')
                    logger.info(f"{now.strftime('%Y-%m-%d %H:%M:%S')} --( 5 Min Data Get Updated )-- ")
            ...
            if self.prev_resistance != self.levels['RESISTANCE'] and self.levels['RESISTANCE'] != float('inf') and not(self.LONG or self.SHORT):
                    ...
                    # logger.info(f'TDSR_Strategy Updated Resistance -( {self.prev_resistance} )-')
                    ...
                    if self.ORDER_ID_BUY == 0:
                        ...
                        self.ORDER_ID_BUY = MT5_buy_order(self.ENTRY, self.STOP_LOSS, self.TARGET, 'TDSR_BUY')

            if self.prev_support != self.levels['SUPPORT'] and self.levels['SUPPORT'] != 0 and not(self.LONG or self.SHORT):
                    ...
                    # logger.info(f'TDSR_Strategy Updated Support -( {self.prev_support} )-')
                    ...
                    if self.ORDER_ID_SELL == 0:
                        ...
                        self.STOP_LOSS = self.ENTRY + self.sl_points
                        self.TARGET = self.ENTRY - (self.sl_points * self.RISK_TO_REWARD)
                        self.ORDER_ID_SELL = MT5_sell_order(self.ENTRY, self.STOP_LOSS, self.TARGET, 'TDSR_SELL')
            ...
            if mt5.positions_get():
                    ...
                    if self.SHORT or self.LONG:
                        ...
                        for open in mt5.positions_get():
                            if open.comment == 'TDSR_BUY':
                                lst_order_buy.append(open.ticket)
                            elif open.comment == 'TDSR_SELL':
    ...
    #         else:
    #             return STOP_LOSS
    # def trailing_stops(long, short, candle_high, candle_low, entry, stop_loss, sl_points, break_even_point,  candle_close):
    #     if long:
    #         if (candle_close - entry) > (sl_points * 4.5):
    #             if entry + (sl_points * 4) > stop_loss:
    #                 return entry + (sl_points * 4)
    #             else:
    #                 return stop_loss
    #         elif (candle_close - entry) > (sl_points * 4):
    #             if entry + (sl_points * 3.5) > stop_loss:
    #                 return entry + (sl_points * 3.5)
    #             else:
    #                 return stop_loss
    #         elif (candle_close - entry) > (sl_points * 3.5):
    #             if entry + (sl_points * 3) > stop_loss:
    #                 return entry + (sl_points * 3)
    #             else:
    #                 return stop_loss
    #         elif (candle_close - entry) > (sl_points * 3):
    #             if entry + (sl_points * 2.5) > stop_loss:
    #                 return entry + (sl_points * 2.5)
    #             else:
    #                 return stop_loss
    #         elif (candle_close - entry) > (sl_points * 2.5):
    #             if entry + (sl_points * 2) > stop_loss:
    #                 return entry + (sl_points * 2)
    #             else:
    #                 return stop_loss
    #         elif (candle_close - entry) > (sl_points * 2):
    #             if entry + (sl_points * 1.5) > stop_loss:
    #                 return entry + (sl_points * 1.5)
    #             else:
    #                 return stop_loss
    #         elif (candle_close - entry) > (sl_points * 1):
    #             if entry + (sl_points * 0.5) > stop_loss:
    #                 return entry + (sl_points * 0.5)
    #             else:
    #                 return stop_loss
    #
    #         elif (candle_close - entry) > (sl_points * break_even_point):
    #             if entry > stop_loss:
    #                 return entry
    #             else:
    #                 return stop_loss
    ...
    #
    #     elif short:
    #         if (entry - candle_close) > (sl_points * 4.5):
    #             if entry - (sl_points * 4) < stop_loss:
    #                 return entry - (sl_points * 4)
    #             else:
    #                 return stop_loss
    #         elif (entry - candle_close) > (sl_points * 4):
    #             if entry - (sl_points * 3.5) < stop_loss:
    #                 return entry - (sl_points * 3.5)
    #             else:
    #                 return stop_loss
    #         elif (entry - candle_close) > (sl_points * 3.5):
    #             if entry - (sl_points * 3) < stop_loss:
    #                 return entry - (sl_points * 3)
    #             else:
    #                 return stop_loss
    #         elif (entry - candle_close) > (sl_points * 3):
    #             if entry - (sl_points * 2.5) < stop_loss:
    #                 return entry - (sl_points * 2.5)
    #             else:
    #                 return stop_loss
    #         elif (entry - candle_close) > (sl_points * 2.5):
    #             if entry - (sl_points * 2) < stop_loss:
    #                 return entry - (sl_points * 2)
    #             else:
    #                 return stop_loss
    #         elif (entry - candle_close) > (sl_points * 2):
    #             if entry - (sl_points * 1.5) < stop_loss:
    #                 return entry - (sl_points * 1.5)
    #             else:
    #                 return stop_loss
    #         elif (entry - candle_close) > (sl_points * 1):
    #             if entry - (sl_points * 0.5) < stop_loss:
    #                 return entry - (sl_points * 0.5)
    #             else:
    #                 return stop_loss
    #
    #         elif (entry - candle_close) > (sl_points * break_even_point):
    #             if entry < stop_loss:
    #                 return entry
    #             else:
    #                 return stop_loss
    ...
    #     #             return stop_loss
    #     #     elif (candle_close - entry) > (sl_points * 2):
    #     #         if entry + (sl_points * 1) > stop_loss:
    #     #             return entry + (sl_points * 1)
    #     #         else:
    #     #             return stop_loss
    #     #     # elif (candle_close - entry) > (sl_points * 1):
    #     #     #     if entry + (sl_points * 1) > stop_loss:
    #     #     #         return entry + (sl_points * 0.3)
    #     #     #     else:
    #     #     #         return stop_loss
    #     #     # elif (candle_close - entry) > (sl_points * break_even_point):
    #     #     #     if entry > stop_loss:
    ...
    def trailing_stops(self, long, short, candle_high, candle_low, candle_close, entry, stop_loss, sl_points,
        ...
        if long:
            ...
            if (candle_close - entry) > (sl_points * break_even_point):
                if entry > stop_loss:
                    return entry
            return stop_loss
            ...
            for rr in [10, 9.5, 9, 8.5, 8, 7.5, 7, 6.5, 6, 5.5, 5, 4.5, 4, 3.5, 3, 2.5, 2, 1.5, 1]:
                if (entry - candle_low) > (sl_points * rr):
                    ...
                    if not (candle_close >= new_stop_loss):
                        if new_stop_loss < stop_loss:
                            return new_stop_loss
            if (entry - candle_close) > (sl_points * break_even_point):
                if entry < stop_loss:
                    return entry
            return stop_loss
